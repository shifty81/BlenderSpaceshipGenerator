...
class BuildValidator:
    ...
    def remove_brick(self, pos):
        """Remove a brick at *pos*."""
        cell = self._to_cell(pos)
        self.placed.pop(cell, None)
        self._positions.pop(cell, None)

    # -- validation ---------------------------------------------------------

    def validate_placement(self, brick_type_name, pos):
        """Validate placing *brick_type_name* at *pos*.

        Returns a :class:`ValidationResult`.
        """
        result = ValidationResult()

        # 1. Brick type must exist
        brick_def = self.brick_types.get(brick_type_name)
        if brick_def is None:
            result.add_error(f"Unknown brick type: {brick_type_name}")
            return result

        # 2. Grid alignment
        if not self._is_aligned(pos):
            result.add_error(
                f"Position {pos} is not aligned to grid "
        ...
        if cell in self.placed:
            result.add_error(
                f"Grid cell {cell} is already occupied by "
                f"{self.placed[cell]}"
            )
