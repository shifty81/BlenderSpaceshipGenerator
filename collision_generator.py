...
def generate_box_collision(hull_obj, naming_prefix=''):
    ...
    xs = [v[0] for v in hull_obj.bound_box]
    ys = [v[1] for v in hull_obj.bound_box]
    zs = [v[2] for v in hull_obj.bound_box]
    ...
    box_obj = bpy.data.objects.new(name, mesh)
    box_obj.scale = (max(size_x, _MIN_BOX_DIMENSION),
                     max(size_y, _MIN_BOX_DIMENSION),
                     max(size_z, _MIN_BOX_DIMENSION))
    box_obj.location = hull_obj.matrix_world @ Vector(
        (center_x, center_y, center_z)
    )

    _link_to_collection(box_obj, hull_obj)
    _set_collision_properties(box_obj, 'BOX', hull_obj.name)
    _setup_display(box_obj)

    box_obj.parent = hull_obj
...
def generate_convex_hull_collision(hull_obj, naming_prefix='',
                                   decimate_ratio=0.15):
    """Duplicate *hull_obj*, decimate it, and return a simplified convex mesh.

    Returns ``None`` if *hull_obj* is invalid or has no geometry.
    """
    if hull_obj is None or hull_obj.type != 'MESH':
        return None
    if len(hull_obj.data.vertices) == 0:
